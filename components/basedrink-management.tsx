...
export const MenuManagement: React.FC = () => {
  ...
  const filteredItems = basedrinks;

  const fetchFlavors = async () => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/flavor/adminget`,
        {
  ...
  const openModal = (item?: BaseDrink) => {
    if (item) {
      setEditingItem(item);
  ...
  const handleSubmit = async (
    ...
    try {
      if (editingItem) {
        await axios.put(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/basedrink/update/${editingItem.id}`,
          {
            name: formData.name,
        ...
        await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/basedrink/create`, {
          name: formData.name,
          price: parseInt(formData.price),
          description: formData.description,
          is_available: formData.is_available,
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/basedrink/delete/${id}`, {withCredentials: true});
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Hold Ctrl/Cmd to select multiple flavors
                </p>
              </div>
